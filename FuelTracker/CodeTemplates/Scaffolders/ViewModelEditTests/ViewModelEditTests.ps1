...
if(!$TestsNamespace)
{
	$TestsNamespace = $defaultNamespace + ".Tests";
}
...
if(!$TestsClassName)
{
	$TestsClassName = $viewModelName + "Tests";
}
...
$outputPath = $outputFolder+ "\" + $TestsClassName  # The filename extension will be added based on the template's <#@ Output Extension="..." #> directive
...
Add-ProjectItemViaTemplate $outputPath -Template ViewModelEditTestsTemplate `
	-Model @{ 
			ModelType = $foundModelType; 
			PrimaryKeyName = $primaryKeyName;
			ViewModelName = $viewModelName;
			ViewModelNamespace = $viewModelNamespace;
			TestsClassName = $TestsClassName;
