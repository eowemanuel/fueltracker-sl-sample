...
{
	throw "T4Scaffolding.Silverlight requires NuGet (Package Manager Console) 1.4 or later"
} 
...
if ($t4ScaffoldingAssemblyVersion -and ($t4ScaffoldingAssemblyVersion -ne "1.0.0.1"))
{
	...
	Write-Warning "---"	
	if (Get-Module T4Scaffolding.Silverlight) {
		# Disable scaffolding as much as possible until VS is restarted
		Remove-Module T4Scaffolding.Silverlight
	}
} 
else 
{
